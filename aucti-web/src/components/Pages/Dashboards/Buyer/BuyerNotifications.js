...
const BuyerNotifications = (props) => {
	const { user, notifications } = props;
	useEffect(() => {
		props.loadNotifications(user.uid);
	}, []);

	const handleRead = (notification) => {
		props.updateNotifications(notification);
	};
...
const mapDispatchToProps = (dispatch) => {
	return {
		loadNotifications: (id) => dispatch(loadNotificationAction(id)),
		updateNotifications: (notification) =>
			dispatch(updateNotificationAction(notification)),
	};
};

export default connect(mapStateToProps, mapDispatchToProps)(BuyerNotifications);
